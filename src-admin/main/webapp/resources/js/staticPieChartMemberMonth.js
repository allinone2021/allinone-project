function pieMemberMonth(objAgeMonth) {
    ...
    objAgeMonth.forEach(result => {
        console.log(result.date)
        console.log(result.customerNum)
        data.push({ name : (result.ageCus), value :(result.customerNum), color : '#f8d690'})
...
// 라벨이 위치할 반지름 값을 설정합니다.
    ...
    const svg = d3.select("#pieChartForMemberMonth").append('svg').style('width', width).style('height', height)
