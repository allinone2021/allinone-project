...
<div class="content-wrapper">
    ...
    <div class="col-md-12">
        ...
        <form action="/notice/modify" method="post" id="modifyForm">
            ...
            <div class="card-body">
                <input type="hidden" name="nno" class="form-control" value="<c:out value="${noticeDTO.nno}"/>">
            ...
            <div>
                ...
                <div class="card-footer">
                    ...
                    <button type="button" class="btn btn-outline-warning btnMod">수정</button>
                    <button type="button" class="btn btn-outline-danger btnDel">삭제</button>
                </div>
            </div>
        </form>
        <style>
            ...
            <c:forEach items="${noticeDTO.files}" var="attach">
                <div data-uuid="${attach.uuid}" data-filename="${attach.fileName}"
                    ...
                    <c:if test="${attach.image}">
                        ...
                        <img class="img-fluid mx-auto"  onclick="javascript:showOrigin('${attach.getFileLink()}')"
                             src="/viewFile?file=${attach.getThumbnail()}">
                            <button class="btn btn-outline-danger" onclick="javascript:removeDiv(this)"> X</button></div>
                    </c:if>
            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                if (image) {
                    str += `<div data-uuid='\${uuid}' data-filename='\${fileName}' data-uploadpath='\${uploadPath}' data-image='\${image}'><img src='/viewFile?file=\${thumbnail}'><span>\${fileName}</span>
                        <button class ="btn "onclick="javascript:removeDiv(this)">x</button></div>`
                } else {
